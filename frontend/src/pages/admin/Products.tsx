...
export default function AdminProducts() {
  ...
  const uploadPreview = async (file: File, productId?: number, quiet = false) => {
    ...
    try {
      ...
      const { data } = await api.post(`/admin/products/${id}/preview`, body);
      setGallery(data.images ?? []);
  ...
  const deleteImage = async (imageId: number) => {
    ...
    setUploading(true);
    try {
      const { data } = await api.delete(`/admin/products/${editingId}/images/${imageId}`);
      setGallery(data.images ?? []);
  ...
  const setPrimaryImage = async (imageId: number) => {
    ...
    setUploading(true);
    try {
      const { data } = await api.post(`/admin/products/${editingId}/images/${imageId}/primary`);
      setGallery(data.images ?? []);
  ...
  const uploadFile = async (file: File, productId?: number, quiet = false) => {
    ...
    try {
      ...
      const { data } = await api.post(`/admin/products/${id}/file`, body);
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1>Products</h1>
          <p className="admin-muted">{products.length} products</p>
        </div>
        <button
...
<table className="data-table card">
          ...
          <tbody>
            {products.map((p) => (
              <tr key={p.id}>
                <td>
                  <img src={imageUrl(p.preview_image)} alt="" className="admin-table-thumb" />
                </td>
