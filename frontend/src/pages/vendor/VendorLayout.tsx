...
const links = [
  { to: '/vendor', label: 'Dashboard', exact: true },
  { to: '/vendor/products', label: 'Products' },
  { to: '/vendor/orders', label: 'Orders' },
  { to: '/vendor/payouts', label: 'Payouts' },
  { to: '/vendor/settings', label: 'Shop settings' },
];

export default function VendorLayout() {
  const { user, isVendor, loading, logout } = useAuth();
  ...
  if (!isVendor) return <Navigate to="/sell" />;

  const isActive = (to: string, exact?: boolean) =>
    exact ? location.pathname === to : location.pathname.startsWith(to);
  const currentLabel = links.find((l) => isActive(l.to, l.exact))?.label ?? 'Vendor';
      ...
      <header className="admin-topbar">
        ...
        <div className="admin-topbar-brand">
          ...
          <div className="admin-topbar-text">
            <p className="admin-sidebar-title">Vendor</p>
        ...
        <div className="admin-sidebar-inner">
          <div className="admin-sidebar-top">
            ...
            <div className="admin-sidebar-brand">
              <img src="/logo.png" alt="The Tailors Market" className="admin-sidebar-logo" />
              <div>
                <p className="admin-sidebar-title">{user.shop?.name || 'Vendor'}</p>
